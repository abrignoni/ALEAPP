...
def open_lava(project_path, launcher):
    ...
    try:
        ...
        tk_msgbox.showerror(
            ...
            message=f'ALEAPP could not open the project in LAVA:\n{ex}',
            parent=main_window)
        return
    main_window.quit()
...
def open_folder(output_path):
    ...
    try:
        ...
        tk_msgbox.showerror(
            ...
            message=f'ALEAPP could not open the output folder:\n{ex}',
            parent=main_window)
...
aleapp_logo = ImageTk.PhotoImage(file=resource_path("ALEAPP_logo.png"))
aleapp_logo_label = ttk.Label(title_frame, image=aleapp_logo)
aleapp_logo_label.pack(side='left')
...
input_frame = ttk.LabelFrame(
    main_window, 
    text=' Select the file (tar, zip, gz, raw image, .E01 acquisition) or directory of the target Android full file system extraction for parsing: ')
