"""Pin what a path under the report folder records as its source path.

`Context.get_relative_path` used to strip the data folder and nothing else, so a path
under the report folder but outside the data folder was returned unchanged and the
examiner's own output directory reached the artifact page's "located at" line and the
LAVA manifest's `source_path`, which travels with the report.

The case that made this live in iLEAPP is an artifact declaring `paths` as None, which
its main script hands '<report folder>/_lava_artifacts.db' as its only files_found
entry. aleapp.py has no such branch and no ALEAPP artifact declares `paths` as None, so
this change alters no ALEAPP output. The five cores carry `get_relative_path` byte for
byte alike, so it is fixed in all of them together rather than left to drift.
