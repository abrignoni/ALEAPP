...
The two are checked differently on purpose. aleapp.py guards its startup with
'if __name__', so it can be imported outright, which catches import-time errors
as well as syntax. aleappGUI.py builds its window at module level, so importing
...
CLI_ENTRY_POINT = REPO_ROOT / 'aleapp.py'
GUI_ENTRY_POINT = REPO_ROOT / 'aleappGUI.py'


class TestEntryPoints(unittest.TestCase):

    def test_cli_entry_point_imports(self):
        """aleapp.py must import, and expose main()."""
        ...
        self.assertTrue(hasattr(module, 'main'), 'aleapp.py no longer exposes main()')

    def test_gui_entry_point_compiles(self):
        """aleappGUI.py must at least compile; it cannot be imported without a display."""
        ...
        try:
            ...
            self.fail(f'aleappGUI.py does not compile on this Python: '
