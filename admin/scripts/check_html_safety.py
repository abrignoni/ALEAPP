...
ALLOWLIST = {
    # The Type cell is assembled from a module-level dict of feather-icon constants
    # keyed by a decoded AOSP call-type code. No evidence text reaches the markup.
    ('scripts/artifacts/calllog.py', 'unguarded-html-columns', '<module>'),

    # Declares a single media column and nothing else, so the cell is built by the
    # framework's media helper and the module has no evidence text of its own.
    ('scripts/artifacts/RandoChat.py', 'unguarded-html-columns', '<module>'),
}
