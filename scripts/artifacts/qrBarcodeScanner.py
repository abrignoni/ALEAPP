__artifacts_v2__ = {
    "qr_barcode_scanner_history": {
        ...
        "notes": "One row per row of the history table in "
                 ...
                 "row; on the tested device it was filled on one row of nine, and on that row it "
                 "held the title of the web page the scanned URL points at. How Details is "
                 "filled on other rows was not exercised, and whether a populated value shows "
                 "that the device fetched the address is not established. Format is the barcode "
                 "symbology the decoder reported. Every code on the tested device was QR_CODE, "
                 "since only QR images were used, and the column is what tells a QR code apart "
                 "from a scanned product barcode. Favorite was 0 and Name was empty on every row "
                 "of the tested image, where nothing was starred or renamed; what fills Name was "
                 "not exercised here. Sort Order is the table's own sorting_order. It held the "
                 "same value as Row ID on every row of the tested image, because the history was "
                 "never reordered there, so the two agreeing is a property of that sample and "
                 "not one column derived from the other. It is kept because a Sort Order that "
                 "departs from Row ID would be visible here; reordering the list was not "
                 "exercised on the tested device. A row does not say where the code came from. "
                 "The app offers "
