__artifacts_v2__ = {
    "linkbox_uploads": {
        ...
        "notes": "com.linkbox.plus.android sends files from the device to a cloud service. The "
                 "store is databases/upload_<account id>.db, one per account, and the account id "
                 "in that file name is reported in its own column; on the corpus below it also "
                 "appeared inside the service side path the same row records, which is why it is "
                 "read as an account id. Local Path is where the file was read from on the "
                 "device, so a row records a file leaving a known location, and on the corpus "
                 "below those locations included the camera folder and the media folders of other "
                 "applications. MD5 is the value the app stored for the file, so a row can be "
                 "matched to a file elsewhere without the file itself being present here. "
                 "create_time, update_time and real_time are Unix milliseconds. Status was 5, "
                 "Error Code 0 and Error Message empty on every row below, so no failed upload is "
                 "represented; no source for the status code list was located and it is reported "
                 "as stored. Use Mobile Data held 0 on every row below, so none of these "
                 "transfers was recorded as having used the mobile network, and the column is "
                 "kept because another value would say one had. The table also carries per-upload "
                 "access key, secret key and session token columns, whose stored times equal the "
                 "row's own create time and so are scoped to that single upload; they are "
                 "credential material that adds nothing to what the other columns already show, "
                 "and they are deliberately not reported. The same database holds "
