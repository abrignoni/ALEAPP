__artifacts_v2__ = {
    "mxplayer_watched": {
        ...
        "notes": "One row per entry in the VideoFile table of "
                 ...
                 "equal to its Last Watched and a Resume Position of 0. A Resume Position of 0 "
                 "is a stored value and not the absence of one; a file with no VideoStates row "
                 "shows the blank described below. Folder comes from the VideoDirectory row the "
                 "file points at, so Folder and File Name together give the path as the app "
                 "stored it. Resume Position is read from the VideoStates table, matched on the "
                 "file URI that table records rather than on any correlation of size or time; a "
                 "file with no VideoStates row shows a blank Resume Position. Decode Mode is "
                 "reported as stored; no source for its values was found. "
    ...
    "mxplayer_media_library": {
        ...
        "notes": "One row per entry in the VideoFile table of "
                 ...
                 "counts are the values the app read out of the file itself. This table does not "
                 "outlive the files it describes: a clip was played to give it a row, the file was "
                 "then deleted and the app reopened, and the row was gone, so the table had five "
                 "rows before and four after. Do not read an entry here as a record of a file that "
                 "once existed, and do not read the absence of one as evidence a file was never "
                 "there; the app prunes on its next scan. Easy Voice Recorder, tested the same way "
                 "on the same device, kept its row, so this behaviour is per app and not a "
                 "property of Android. On the tested image two of the four rows were files left by "
                 "other apps "
                 "and never opened in MX Player, and both carried a Duration and a resolution but "
                 "no track counts, while the two played files carried them. That is also why "
                 "Video Tracks and Audio Tracks carried the same "
