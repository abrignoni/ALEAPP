__artifacts_v2__ = {
    "flipboard_article_history": {
        ...
        "notes": "One row per row of the view_history table in "
                 ...
                 "reported as UTC. Each row also carries a JSON copy of the item as it stood "
                 "when it was opened, in the valid_item column, and Source URL, Author, "
                 "Published and Excerpt are read out of that JSON rather than fetched from "
                 "anywhere. Published is the item's own dateCreated field, Unix seconds, and is "
                 ...
                 "not truncated by this artifact. Source URL is reported as plain text and is "
                 "not made clickable. Read and Bookmarked come from the isRead and isBookmarked "
                 "flags in the same JSON. The table does not record how long an item stayed open "
                 "or whether it was read; what action writes a row was not sourced. Flipboard "
                 "runs without an "
    ...
    "flipboard_sections": {
        ...
        "notes": "One row per row of the sections table in flipboard.app/databases/users-v6.db. "
                 ...
                 "and Private come from that JSON. On the tested device rows with Feed Type "
                 "topic carried a Remote ID of the form flipboard/topic%2F followed by the topic "
                 "slug. Rows with no Feed Type are the feeds the app added, and their Remote ID "
                 "names what they are: the tested device held auth/flipboard/coverstories and "
                 "one flipboard/mix row. Position is the sections table's own pos column, "
                 "reported as stored. There is no timestamp in this table, so a row says the "
                 "section is followed and not when it was followed. On the tested device the "
                 "three topics chosen during first run were all present with Feed Type topic, "
                 "alongside those two. Service held the single value flipboard on every row and "
                 "User ID held one value, because the tested device carried one anonymous "
                 "profile and no other service. Both are kept as columns; a device with more "
                 "than one profile or service was not exercised.",
