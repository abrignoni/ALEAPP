...
def get_installedappsVending(files_found, report_folder, seeker, wrap_text):
    ...
    columns = [i[1] for i in cursor.execute('PRAGMA table_info(appstate)')]
    
    if 'install_reason' in columns:
        cursor.execute('''
        SELECT
        ...
        if usageentries > 0:
            ...
            report.start_artifact_report(report_folder, 'Installed Apps (Vending)')
            report.add_script()
            data_headers = ('First Download','Package Name', 'Title', 'Last Updated', 'Auto Update?', 'Account')
            data_list = []
            for row in all_rows:
