__artifacts_v2__ = {
    ...
    "opentracks_trackpoints": {
        ...
        "notes": "One row per entry in the trackpoints table of databases/database.db, joined to "
                 ...
                 "temperature) were empty on the tested device, which had no sensor paired; they "
                 "are kept because their absence "
    ...
    "opentracks_markers": {
        ...
        "notes": "One row per entry in the markers table of databases/database.db, joined to its "
                 "track. Each row carries a Name, Description, Marker Type as stored, "
                 "coordinates, Altitude, Accuracy and Bearing, and an optional Photo URL. "
                 "Latitude and Longitude are divided by 1E6 as in the Trackpoints artifact, and "
                 "Time is Unix milliseconds reported as UTC with a local rendering from the "
                 "parent track's recorded offset. Whether a marker row records a deliberate act "
                 "was not exercised here, since no marker was present on the tested device. The "
                 "markers table was present "
