__artifacts_v2__ = {
    ...
    "elementx_sessions": {
        ...
        "notes": "One row per Matrix SDK store found for a session. Element X keeps one directory per "
                 ...
                 "which bounds the scale of activity, and not as content. The content was not "
                 "recoverable from the tested logical extraction: the app generates its database "
                 "secret with SecureRandom and writes it to an AndroidX EncryptedFile "
                 "(RandomDatabaseSecretProvider.kt), and that file's Tink keyset is encrypted "
                 "with a master key held in the Android Keystore (EncryptedFile.kt, both at "
                 "element-hq/element-x-android c47446bfb974bb5abce9f2938fafd71a52a1a5b6); the "
                 "observed device matched this, holding an AesGcmHkdfStreamingKey keyset in "
                 "shared_prefs and an 88 byte session_database.key beside an unreadable "
                 "session_database.db. No Keystore key was present in the extraction, so message "
                 "text, room names and member lists stayed encrypted. This was confirmed on the "
                 "tested device by writing a known message and "
