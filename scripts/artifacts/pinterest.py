__artifacts_v2__ = {
    "pinterest_account": {
        ...
        "notes": "Read from the PREF_MY_USER value of the app's own preferences file, which holds "
                 ...
                 "midday UTC, and whether the time part carries meaning is not established. A "
                 "birth date before 1970 is stored "
    ...
    "pinterest_cached_images": {
        ...
        "notes": "Covers both DiskLruCache stores the app keeps for images, the one under "
                 ...
                 "entry length and carries no URL; the entry key is not a hash of the URL as far "
                 "as was checked: SHA-256, SHA-1, MD5 and UTF-16 spellings of the four profile "
                 "image URLs the account record does hold matched none of the 6300 entry names "
                 "on the tested sample; and the MD5 of every cached file matched none of the MD5 "
                 "values embedded in those URLs, so the cached bytes are not the files those "
                 "URLs name. The entry key is "
    ...
    "pinterest_cached_videos": {
        ...
        "notes": "Two stores are reported and the Source Store column says which each row came "
                 ...
                 "sample. Those fragments are partial byte ranges of the media rather than "
                 "playable files, "
