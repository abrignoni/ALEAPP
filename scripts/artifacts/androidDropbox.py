__artifacts_v2__ = {
    ...
    "dropbox_process_errors": {
        ...
        "notes": "Read from the system dropbox entry files whose tag is <class>_<event>, where "
                 ...
                 "SYSTEM_TOMBSTONE_PROTO, filed as the bare Tombstone message at "
                 "android-13.0.0_r1 (BootReceiver.addTombstoneToDropBox), and "
                 "SYSTEM_TOMBSTONE_PROTO_WITH_HEADERS, filed wrapped in "
                 "TombstoneWithHeadersProto (tombstone 1, dropped_count 2) on later releases; "
                 "the release at which each form starts and stops was not established here. The "
                 "Tombstone field numbers are build_fingerprint 2, timestamp 4, pid 5, "
    ...
    "android_tombstones": {
        ...
        "notes": "Read from the tombstone_<n> text files and their tombstone_<n>.pb protobuf "
                 ...
                 "record read on the tested images. This is the set of tombstone files present "
                 "at acquisition, not necessarily a full crash history. On "
