__artifacts_v2__ = {
    "permission_app_op_modes": {
        ...
        "notes": "A row is an app-op element in the permission subsystem's access.abx carrying a stored mode, with "
                 ...
                 "direction only. Every one of the 22 images carrying access.abx reports Android 15 or later, and "
                 "none below that carries it. It is not the case that system/appops.xml disappears at that "
                 "release: of the 22 tested images at Android 15 or later, 1 still carries it, a Samsung, and on "
                 "that one both files are live. Of the 40 registered corpora, 16 carry system/appops.xml, 1 "
                 "carries both files and 3 carry neither, those last being partial extractions with no system "
                 "files. Measured against the registry as it stood on 2026-09-07, across all 40 registered Android "
                 "corpora, 22 carry access.abx, and those are the same 22 the runs produced rows on. The store is "
                 "written once for the system, with no Android user, and once per Android user, "
    ...
    "permission_grants": {
        ...
        "notes": "A row is a permission element in the app-id-permissions section of the permission subsystem's "
                 ...
                 "access.abx, and those are the same 22 the runs produced rows on. The store is written once for "
                 "the system, with no Android user, and once per Android user, AccessPersistence.writeSystemState "
                 "and writeUserState, both named access.abx, "
                 "https://android.googlesource.com/platform/frameworks/base/+/1cdfff555f4a21f71ccc978290e2e212e2f8b168/services/permission/java/com/android/server/permission/access/AccessPersistence.kt#156 "
                 "and #160, in the directories PermissionApex names for the com.android.permission module, #173 "
                 "and #175. On the tested images those are misc/apexdata/com.android.permission for the system and "
                 "misc_de/<user>/apexdata/com.android.permission for each user.\n"
