__artifacts_v2__ = {
    "permission_app_op_modes": {
        ...
        "notes": "A row is an app-op element in the permission subsystem's access.abx carrying a stored mode, with "
                 ...
                 "with it how Package Name was obtained: 3,539 rows are stored against an app id and 2,171 against "
                 ...
                 "reads. From Android 15 that file is gone on the tested images and the modes are here: of the 39 "
                 "registered corpora, 16 carry system/appops.xml, 1 carries both files and 3 carry neither, those "
                 "last being partial extractions with no system files. Measured across the 39 registered Android "
                 "corpora on 2026-09-06, 21 carry access.abx, and those are the same 21 the runs produced rows on. "
                 "The store is written once for the system, with no Android user, and once per Android user, "
                 "AccessPersistence.writeSystemState and writeUserState, both named access.abx, "
                 ...
                 "Checked against the data rather than taken on the source's word: of the 5,500 stored modes "
                 "across the tested images whose op AOSP's own table defines a default for, 5,500 differed from "
                 "that default and 0 matched it.\n"
                 "Mode is the AppOpsManager mode constant, and a value outside that set is reported as stored. "
                 "Across the tested images the stored modes were IGNORED on 2,911 rows, DEFAULT on 1,515 rows, "
                 "ALLOWED on 929 rows, FOREGROUND on 304 rows, ERRORED on 2 rows. The value 5 on 49 rows is "
                 "reported as stored: AOSP defines modes 0 to 4 and no more. Every row carrying it is on one of "
                 "the 1 tested images and pairs with an op string in a vendor namespace rather than the android "
                 "one. Reference: MODE_ALLOWED, MODE_IGNORED, MODE_ERRORED, MODE_DEFAULT and MODE_FOREGROUND, "
                 ...
                 "UNARCHIVAL_CONFIRMATION, which is on 36 rows here. Op Code is blank for 210 rows carrying 12 op "
                 ...
                 "appear on 3 of the 21 images with rows, and no source for them was found, so they are reported "
                 "as stored.\n"
                 ...
                 "value was 0 on 4,110 rows, 10 on 1,600 rows.\n"
                 ...
                 "Package Name is blank on 0 of 5,710 rows.\n"
                 ...
                 "paths, while two copies that disagree produce a row each. On the 21 images carrying the store, "
                 "every Android 15 emulator carries a data_mirror copy of each user file, and all 34 of those 34 "
                 ...
                 "writes with no Android user, which is present on 21 of those 21 images. It is read and named in "
                 ...
                 "sections into it, so it is left broad deliberately. 3,793 of 5,710 rows cite more than one path.\n"
                 ...
                 "held one value per file on every one of the 72 user files read, so it says nothing about an "
                 "individual package. default-permission-grant is not reported: it is one build fingerprint per "
                 "file, which ALEAPP already reports from the device's own build properties. The system file's "
                 "permissions and permission-trees sections are not reported either: they are the permission "
                 "declarations the platform and the installed apps make in their manifests, 1,442 to 3,060 per "
                 "image from 357 declaring packages on the image with the most, which exist whether or not anyone "
                 "used anything. app-id-device-permissions was present and empty on all 72 user files read, and "
                 ...
                 "That is much tighter than the 30 minute delay on system/appops_accesses.xml, so the contents are "
                 "close to the state at acquisition, but they are still the last write and not a live view.",
    ...
    "permission_grants": {
        ...
        "notes": "A row is a permission element in the app-id-permissions section of the permission subsystem's "
                 ...
                 "187,278 rows across the tested images, carrying 4,508 distinct permission names. Reference: "
                 ...
                 "presence is not evidence that it still holds anything: on 20 of those 21 images it is a stub of "
                 "981 to 1,160 bytes carrying no permission element at all. On the remaining one, a Samsung "
                 ...
                 "stores should not be treated as interchangeable on a device that writes both. Measured across "
                 "the 39 registered Android corpora on 2026-09-06, 21 carry access.abx, and those are the same 21 "
                 "the runs produced rows on. The store is written once for the system, with no Android user, and "
                 "once per Android user, AccessPersistence.writeSystemState and writeUserState, both named "
                 "access.abx, "
                 "https://android.googlesource.com/platform/frameworks/base/+/1cdfff555f4a21f71ccc978290e2e212e2f8b168/services/permission/java/com/android/server/permission/access/AccessPersistence.kt#156 "
                 "and #160, in the directories PermissionApex names for the com.android.permission module, #173 "
                 "and #175. On the tested images those are misc/apexdata/com.android.permission for the system and "
                 ...
                 "stays visible, which 0 of the 187,278 tested rows needed. Reference: "
                 ...
                 "PROTECTION_GRANTED on 76,383 rows, INSTALL_GRANTED on 61,599 rows, USER_SENSITIVE_WHEN_GRANTED "
                 "on 23,383 rows, RUNTIME_GRANTED on 18,729 rows, USER_SENSITIVE_WHEN_REVOKED on 18,634 rows, "
                 "INSTALL_REVOKED on 14,606 rows, PREGRANT on 11,959 rows, UPGRADE_EXEMPT on 8,707 rows, "
                 "SYSTEM_FIXED on 6,793 rows, ROLE on 4,524 rows, SYSTEM_EXEMPT on 4,453 rows, INSTALLER_EXEMPT on "
                 "2,497 rows, IMPLICIT on 1,254 rows, USER_SET on 1,049 rows, IMPLICIT_GRANTED on 85 rows, "
                 ...
                 "It was Yes on 155,622 rows and No on 31,656. AOSP carries a second, stricter function, "
                 ...
                 "Not every row is a decision somebody made. INSTALL_GRANTED, INSTALL_REVOKED and "
                 "PROTECTION_GRANTED are the state the platform derives from an app's manifest and the "
                 "permission's protection level at install time, and a row carrying only those exists on any "
                 "device with that app installed: 151,509 of 187,278 rows are that shape. The rows a user's own "
                 "choices reach are the ones carrying a runtime flag, and the narrowest of those are USER_SET on "
                 "1,049 rows, USER_FIXED on 3, ONE_TIME on 27 and USER_SELECTED on 69. The whole table is reported "
                 "rather than the runtime subset, so an absent row means the subsystem held no state for that app "
                 "id and permission rather than this artifact having filtered it, and the flag names are in one "
                 "column so the narrower sets can be found by sorting on it.\n"
                 "Android User is read from the path, because the file does not store it: AOSP writes one file per "
                 "user into that user's own directory. A row from the system file would carry no user, and none "
                 "does, because the system file holds none of the sections read here. Across the tested images the "
                 "value was 0 on 113,692 rows, 10 on 73,586 rows.\n"
                 "Package Name is resolved from system/packages.xml, not stored in this file. That lookup uses "
                 "identity the file records: the userId attribute of a package element, or the name of the "
                 "shared-user element when several packages share the app id, in which case the name is a shared "
                 "user id such as android.uid.system rather than a package. A uid is reduced to an app id first, "
                 "https://android.googlesource.com/platform/frameworks/base/+/1cdfff555f4a21f71ccc978290e2e212e2f8b168/core/java/android/os/UserHandle.java#47. "
                 "Package Name is blank on 0 of 187,278 rows.\n"
                 "The path pattern is tolerant, so every copy of the file in an extraction is read. A row is keyed "
                 "on its content, so a copy that repeats a record costs no extra row and Source File names both "
                 "paths, while two copies that disagree produce a row each. On the 21 images carrying the store, "
                 "every Android 15 emulator carries a data_mirror copy of each user file, and all 34 of those 34 "
                 ...
                 "declarations and none of the per-user sections. Narrowing the pattern to the per-user directory "
                 "would drop those citations, and would also drop the file if a later release moved one of these "
                 "sections into it, so it is left broad deliberately. 148,420 of 187,278 rows cite more than one "
                 "path.\n"
                 ...
                 "granted for a virtual device, was present and empty on all 72 user files read.",
        "paths": ('*/apexdata/com.android.permission/access.abx', '*/system/packages.xml'),
        "output_types": "standard",
        "artifact_icon": "shield",
        "sample_data": {
