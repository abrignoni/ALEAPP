__artifacts_v2__ = {
    "google_play_movies_account": {
        ...
        "notes": "Source is purchase_store.db. On 9 of the 18 database copies tested the database "
                 ...
                 "nothing about when or whether any of them was used. user_configuration holds "
                 "one row per account carrying a configuration blob. 'Has Configuration Record' "
                 "is Yes when a row for that account is present in user_configuration and No when "
                 "it is not; the store does not record why a row is absent, and an absent row is "
                 "not evidence about how the account was used. Across the 16 tested images 19 "
                 "accounts are listed and 18 of them have a configuration record, the one "
                 "exception being on samsunga53_a14. The value is left blank if "
                 "user_configuration.config_account cannot be read, so a read failure is not "
                 "reported as a No. The column was readable on all 18 copies tested, so no image "
                 "exercised that branch; it was verified against a constructed copy with the "
                 "table removed, which reported the account with a blank value and logged the "
                 "reason. The country is likewise left blank and the reason logged when "
                 "config_proto is absent, verified the same way against a copy carrying the table "
                 "without that column. 'Country Code' is the first field of the "
                 "user_configuration.config_proto protobuf, reported as stored and not validated "
                 "against a shape. It held a two letter code for every account that has a "
                 "configuration record, and is blank for the one that does not; US on 18 of the "
                 "20 account rows of the tested images, CH on galaxys10_a10 and blank on the one "
                 "row without a configuration record. Three values agree on that copy: the field "
                 "itself, the rating system carried in the same blob (FSK/CH, against the TVPG "
                 "and MPAA/US carried by the US copies), and the wifi country code the device's "
                 "own Android settings record independently of this app. What Google uses the "
                 "field for is not documented and is not claimed here. The app's own "
                 "user_configuration.config_play_country column is a different field and was NULL "
                 "for every account tested. Source File carries the evidence relative path of the "
                 "database each row came from rather than a derived profile number, so an account "
                 "held under a second Android user is attributable without inferring anything "
                 "from the path: russell_a14 and russell_pixel6a_a13 each carry a second copy "
                 "under data/user/10 holding a different account from the copy under data/data. "
                 "This artifact reports account identity only. Of the 23 tables in this database, "
