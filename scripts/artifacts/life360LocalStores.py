...
__artifacts_v2__ = {
    "life360DeviceLocationStore": {
        ...
        "notes": "Read from the location table of "
                 "com.life360.android.safetymapd/databases/L360LocationLocalStoreRoomDatabase, a "
                 "store in the app's own data directory; the -wal and -shm sidecars are read "
                 "with it because 12 and 11 of the rows on two tested images existed only in the "
                 "write-ahead log. Timestamp is the time column, a millisecond Unix epoch, "
                 "converted to UTC. Sample Type (as stored) is the row's type (RAW 109, FILTERED "
                 "94 and SENT 55 of the 258 rows on the 3 tested images that held the store), "
                 "Location Mode (as stored) is the lmode value (int, move, prox, geo, gh or "
                 "heartbeat, blank on 62 rows), and Provider (as stored) was fused on every row "
                 "as User Activity (as stored) was unknown on every row; both are kept as the "
                 "values the app wrote. Bearing (as stored) was 0 on 198 of the 258 rows and "
                 "Speed (as stored) 0 on 76, and Battery Charging (as stored) and Wi-Fi "
                 "Connected (as stored) are the 0 or 1 flags stored with the fix, each one value "
    ...
    "life360CircleDevices": {
        ...
        "notes": "Read from the devices table of the same MembersEngineRoomDatabase, with Circle IDs from "
                 ...
                 "that held one value per image. Device Type (as stored) was PHONE "
