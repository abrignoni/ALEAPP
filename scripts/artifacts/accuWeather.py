__artifacts_v2__ = {
    "accuweather_location": {
        ...
        "notes": "One row per location record in the DataStore preferences file "
                 ...
                 "so the key can name somewhere the device has already left. The app resolves "
                 "it through a lookup rather than deriving it locally, which is the likely "
                 "reason, though that was not established here. "
                 "Default Location is which source the app was set to use, 'gps_location' on "
                 "the tested image. "
                 ...
                 "A row records the position the app last held, not a track. That the file keeps "
                 "one coordinate pair and overwrites it was measured rather than assumed: "
                 "moving the device twice replaced the pair both times and the file shrank "
                 "from 285 to 284 to 283 bytes, never growing, so no previous position is "
                 "retained and there is no way to tell how long the device was anywhere. "
