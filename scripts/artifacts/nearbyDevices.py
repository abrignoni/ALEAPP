__artifacts_v2__ = {
    "nearby_fast_pair": {
        ...
        "notes": "Read from the Play services LevelDB store "
                 ...
                 "observation is the later of the two. Most records carry neither: they were on 1 of "
                 "7 records on one tested image and none of 15 on another, so the table leads with the "
                 "address rather than a time and Last Observed and First Observed are often blank.\n"
                 "Device Name is the model the accessory reports and Title is the name shown for "
                 "it, which carried an owner-given name on the tested images, so the two are "
                 "reported separately. State is reported as stored. The record also holds the "
                 "accessory's image, which is reported only as a byte count, and a short binary "
                 "account key that is not reported. On an image whose records are all for one accessory, "
                 "MAC Address, Manufacturer, Model ID and Image Bytes each hold a single value; they are "
                 "kept as columns because an image with several accessories separates them.\n"
