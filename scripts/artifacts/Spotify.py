__artifacts_v2__ = {
    ...
    "spotify_now_playing_view": {
        ...
        "notes": "Source is the app's HTTP disk cache (cache/http-cache): the "
                 ...
                 "device would not generalise to any other extraction. 'Cache File "
                 "Time' is the response file's own filesystem modification time (the "
                 "same convention this project already uses for browser cache "
                 "artifacts, e.g. browserCachechrome.py) - it is when this API "
                 "response was cached locally, which is a reasonable proxy for when "
                 "the track was on screen but is not itself a playback timestamp.",
        "paths": ('*/com.spotify.music/cache/http-cache/*',),
        "output_types": ["standard"],
    ...
    "spotify_artist_profile_views": {
        ...
        "notes": "Source is the app's HTTP disk cache (cache/http-cache), "
                 ...
                 "'artistUri' fields. 'Cache File Time' is the response file's own "
                 "filesystem modification time (the same convention this project "
                 "already uses for browser cache artifacts) - it is when this "
                 "profile response was cached locally, which is a reasonable proxy "
                 "for when the page was opened but is not itself confirmation of "
                 "how long the page was viewed or that each track by that artist "
                 "was played.",
        "paths": ('*/com.spotify.music/cache/http-cache/*',),
        "output_types": ["standard"],
...
def spotify_now_playing_view(context):
    ...
    for track_id, response_path, request_path in lyrics_entries:
        ...
        data_list.append((
            ...
            _cache_file_time(response_path),
        ))
...
def spotify_artist_profile_views(context):
    data_headers = (
        "Artist Name", "Artist URI", ("Cache File Time", "datetime"),
    )
    ...
    for paths in pairs.values():
        ...
        if not name and not artist_uri:
            continue
        data_list.append((name, artist_uri, _cache_file_time(response_path)))
