__artifacts_v2__ = {
    "package_user_states_orphaned": {
        ...
        "notes": "A row is a user-states element inside the domain-verifications-legacy section of "
                 "packages.xml whose packageName has no package and no updated-package record left in "
                 "the same file. One row per package name, Android user and stored state, so a row count "
                 "is not a count of packages: on one tested image 69 such package names produced 135 "
                 "rows because most were recorded for two Android users.\n"
                 ...
                 "A row is therefore evidence that the package name was recorded on the device. The "
                 "absence of a row is not evidence that a package was never present. writeSettings skips "
                 "an entry that holds no per-user state, so a package the legacy store never recorded "
                 "state for has no element whether or not its record was later removed. Measured on an "
                 "Android 15 emulator on 2026-09-06: of four packages removed with pm uninstall, three "
                 "left an element and the fourth had none either before or after the removal, and the "
                 "reason for the difference was not established. Nothing in the element carries a "
                 "timestamp, so this artifact does not report when a package record was removed.\n"
                 ...
                 "com.google.android.trichromelibrary packages, one of which is installed and removed "
                 "per browser update, and most of the remaining 126 are platform, vendor and resource "
                 "overlay packages that a system update replaced. Third party application packages also "
                 "appear, and they are the reason to read the table.\n"
                 ...
                 "state carried by both produces one row naming both files. On the tested images the "
                 "reserve copy carried no package name the live file did not, so it adds no rows. It is "
                 "also a fallback: on an input built from a readable reserve copy beside an unreadable "
                 "live file, the rows were read from the reserve copy and cite it alone. Samsung devices "
                 "also keep older snapshots of packages.xml under "
                 ...
                 "The residue is specific to packages.xml. The per-user package-restrictions.xml files "
                 "were checked on three tested images carrying 8, 69 and 111 orphaned package names, and "
                 "named none of them among the 407, 383 and 541 packages they list.\n"
