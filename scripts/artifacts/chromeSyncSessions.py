__artifacts_v2__ = {
    "chrome_sync_sessions": {
        ...
        "notes": "Read from the Chrome sync LevelDB store app_chrome/<profile>/Sync Data/LevelDB "
                 ...
                 "keys beginning sessions-dt- are read; the sessions-md- keys are not read. The "
                 "mirrored Android storage views of one store are collapsed before it is opened "
                 "so a record is not reported once per view.\nField numbers are Chromium's. "
                 "SessionSpecifics: session_tag 1, header 2, tab 3, tab_node_id 4. "
                 "SessionHeader: window 2, client_name 3, device_type 4. SessionTab: tab_id 1, "
                 "window_id 2, current_navigation_index 4, pinned 5, navigation 7. "
                 "TabNavigation: virtual_url 2, referrer 3, title 4, page_transition 6, "
                 "unique_id 8, timestamp_msec 9, favicon_url 17, http_status_code 20. "
                 "References: Chromium, components/sync/protocol/session_specifics.proto and "
                 "components/sync/protocol/tab_navigation.proto.\nOne row per navigation entry in "
                 "a tab. Timestamp is timestamp_msec, Unix milliseconds. Device Name is the "
                 "client_name from the header record carrying the same session tag, so a tab is "
                 "attributed to the device that synced it and is blank when no header for that "
                 "tag is in the store. Browser is the package the store sits under, because the "
                 "pattern carries no package name and is anchored on the app_chrome layout "
                 "rather than on a package. Navigation Index is the entry's position in the "
                 "tab's list and Current Navigation Index is the entry the tab was on, so the "
                 "two together show which page was showing. Page Transition is an integer "
                 "Chromium defines and is reported as stored. Favicon URL is carried by a "
                 "minority of entries, 35 of 2551 across the tested images, and is blank on the "
                 "rest.\nSync carries tabs from the other devices signed into the same Chrome "
                 "profile, so a row is not necessarily a page opened on this device; read Device "
                 "Name before attributing it. The LevelDB store can hold superseded copies of a "
                 "key; Superseded is True for every copy but the newest of that key, and an "
                 "earlier copy can hold "
    ...
    "chrome_sync_devices": {
        ...
        "notes": "Read from the same Chrome sync LevelDB store, from the session records that "
                 ...
                 "it was used at the device this extraction came from. The store can hold "
                 "superseded copies of each header; only the newest copy of each is reported "
                 "here, and the synced tabs artifact reports the "
