__artifacts_v2__ = {
    "kmplayer_playback": {
        ...
        "notes": "One row per live entry of the Hive box "
                 ...
                 "it. The reader addresses a stored object by numbered field, and the numbers "
                 "were mapped against playback created deliberately on the tested device rather "
                 "than read from any source. Two items were played: a local video opened by "
                 "content URI and a video fetched from an http address, which is what lets the "
                 "fields be told apart. Opened is field 10, Unix milliseconds, reported as UTC; "
                 "both rows matched the wall clock of the play that produced them to the second. "
                 "Address is field 3, which held the full content:// URI for the local play and "
                 "the full http URL for the remote one, so on the remote row it records an "
                 "address the device contacted. Title is field 1; on the tested image it held "
                 "the file name on the remote row and the MediaStore id on the local row, so it "
                 "is not always a human title. Source (as stored) is field 0; it read 0 on the "
                 "local row and 1 on the remote row of the tested image, which is one "
                 "observation of each and not enough to state what the field means, so it is "
                 "reported as stored. A row records that the app opened the item, not that "
                 "anyone watched it, and not how much was played. Other stores in the same app "
                 "that are not parsed here: hive_media_flag_box holds a per-item playback flag "
                 "block whose field meanings could not be established from two entries; "
                 "hive_setting_box, hive_home_group_box and hive_quickbutton_box held app "
                 "configuration and menu layout on the tested image; hive_mylist_box, "
