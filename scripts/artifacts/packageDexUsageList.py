__artifacts_v2__ = {
    "package_dex_usage_list_cross_package": {
        ...
        "notes": "Read from the platform's dex use store at /data/system/package-dex-usage.list. "
         ...
         "version 2. This artifact reports the \"+\" primary code path records, which are "
         "installed APKs and their splits, one row for each code path and loading package. The "
         "store does not record a package loading its own primary code: "
         "DexManager.notifyDexLoadInternal skips the record when the file is a primary or split "
         "APK and the loading package equals the owning package, an exemption that is "
         "unconditional at android-10.0.0_r1 and gated on the platform package \"android\" from "
         "android-11.0.0_r1. The store therefore has no equivalent of the protobuf store's "
         ...
         "Package. Those are code paths whose recorded loaders have all been removed: "
         "PackageDexUsage.syncData drops a loading package that is no longer installed while "
         "keeping the code path, and DexManager.loadInternal calls syncData at every boot. They "
         "are reported with the column blank rather than dropped, and a blank there is not "
         "evidence that nothing loaded the file. Isolated Process is true when the recorded "
         "loading package carried DexManager's ISOLATED_PROCESS_PACKAGE_SUFFIX \"..isolated\", "
         "which the platform appends so that a load from an isolated process counts as coming "
         "from a different package; the Loading Package column reports the name without that "
         "suffix. The suffix was added at android-12.0.0_r1 and is absent from android-10.0.0_r1 "
         "and android-11.0.0_r1. It was set on 1 row of the 1058 read here, and that row is the "
         "only one whose loading package equals its owning package once the suffix is removed. "
         "One package loading another's code is ordinary platform behaviour more often than not: "
         "477 of the 1058 rows name Google Play services as the owning package and 140 the "
         "Android System WebView, which apps load to render web content, so the row worth "
         "attention is an unexpected pairing rather than the presence of pairings. 662 of the "
         "rows load a file under /data and 396 a platform path under /apex, /product, /system, "
         "/system_ext. The format records no time. There is no timestamp field anywhere in the "
         "writer, so a row records that a load happened at some point before the file was last "
         "written and says nothing about when. That is the difference from the protobuf store at "
         "/data/system/package-dex-usage.pb that replaces it, whose records each carry a "
         "last_used_at_ms. Only the file's own modification time bounds the store, and it bounds "
         "the whole file rather than any row. The platform writes through "
         "AbstractStatsBase.maybeWriteAsync, whose WRITE_INTERVAL_MS leaves at least 30 minutes "
         "between background writes, so a load in the last half hour before acquisition can be "
         "absent. An entry does not outlive the package it names. PackageDexUsage.syncData "
         "removes the record of an owning package that is no longer installed, removes secondary "
         "dex entries for a user that no longer exists, removes a code path the owning package "
         "no longer declares, and removes the whole package record once nothing is left; "
         "DexManager.loadInternal calls it at every boot, passing an empty list of packages to "
         "keep data about, so nothing is exempted. That is read from the source and was not "
         "exercised against a device here, so a name present is evidence the package was "
         "installed at the last boot rather than at acquisition, and a name absent is not "
         "evidence the package was never installed. Across the 39 registered Android corpora the "
         "file is populated on 13 images, which report Android 10, 11, 12, 13 and 14. 4 images "
         "reporting Android 14 and 15 carry it reduced to its 38 byte header line, and it is "
         "absent from 17 Android 15 emulator images and 2 images reporting Android 16 and 17. "
         "The remaining 3 corpora are extractions that carry no /data/system content at all "
         "rather than devices without the store, which was checked by listing their members. The "
         "store is not simply superseded at Android 14: DexManager.notifyDexLoadInternal still "
         "calls PackageDexUsage.record at android-14.0.0_r1, and 2 of the images reporting "
         "Android 14 carry a populated file whose recorded modification time is within days of "
         "the extraction. Why the file stops being maintained on some Android 14 and 15 devices "
         "and not others was not established here, and because the format carries no times it "
         "cannot be settled from the file itself. One set of rows is reported per store found, "
         "each row carrying its own Source File, so an extraction holding the folder under more "
         "than one root reports each copy rather than merging them; every corpus read here held "
         "exactly one copy, and the two-copy case was exercised on a constructed tree. The path "
         "pattern is not anchored on a data/ prefix, so a raw userdata partition image that "
         "carries system/ at its root is matched as well, which is how one of the images read "
         "here is laid out. A file matching the pattern that does not begin with the format's "
         "header line is logged and not read. The two Dex Usage (Legacy) artifacts partition the "
         "file without overlap: this one and Secondary Dex Loads. Every line of the file is "
    ...
    "package_dex_usage_list_secondary": {
        ...
        "notes": "Read from the platform's dex use store at /data/system/package-dex-usage.list. "
         ...
         "read here confirms. PACKAGE_DEX_USAGE_VERSION is 2, which the class's own comment "
         "dates to Oreo with version 1 support dropped in R, and every store read here carried "
         "version 2. This artifact reports the \"#\" secondary dex records, one row for each dex "
         "or jar file a package loaded from outside its own installation. Android User is the "
         "owner user id the platform recorded; it was 0 on 1114 of the 1227 rows read here and "
         "named a second Android user on 113, so the column separates a second profile's "
         "activity from the first user's. Also Loaded By lists the packages on the record's \"@\" "
         "line, which excludes the owning package by construction, because "
         "PackageDexUsage.maybeAddLoadingPackage adds a loading package only when it differs "
         "from the owner. An empty Also Loaded By therefore means only the owning package was "
         "recorded as loading the file, not that nothing loaded it; 134 of the 1227 rows name "
         "another package. The record also carries a used-by-other-apps flag, which "
         "PackageDexUsage.writeBoolean writes as 1 or 0. It is not reported as its own column: "
         "the writer sets it from the same comparison that decides whether a loader is added, "
         "and on all 1227 rows read here it was true exactly when Also Loaded By is not empty, "
         "so a separate column would repeat that one. Instruction Sets holds the instruction "
         "sets the platform recorded for the load, arm64 on 1221 rows and arm on 19 rows. Class "
         "Loader Context is the encoding as stored; \"=VariableClassLoaderContext=\" is the marker "
         "PackageDexUsage.VARIABLE_CLASS_LOADER_CONTEXT writes when a file was loaded under more "
         "than one context, and it was the value on 225 of the 1227 rows, so on those rows the "
         "column names no classpath. The platform caps a package at "
         ...
         "least 30 minutes between background writes, so a load in the last half hour before "
         "acquisition can be absent. An entry does not outlive the package it names. "
         "PackageDexUsage.syncData removes the record of an owning package that is no longer "
         "installed, removes secondary dex entries for a user that no longer exists, removes a "
         "code path the owning package no longer declares, and removes the whole package record "
         "once nothing is left; DexManager.loadInternal calls it at every boot, passing an empty "
         "list of packages to keep data about, so nothing is exempted. That is read from the "
         "source and was not exercised against a device here, so a name present is evidence the "
         "package was installed at the last boot rather than at acquisition, and a name absent "
         "is not evidence the package was never installed. Across the 39 registered Android "
         "corpora the file is populated on 13 images, which report Android 10, 11, 12, 13 and "
         "14. 4 images reporting Android 14 and 15 carry it reduced to its 38 byte header line, "
         "and it is absent from 17 Android 15 emulator images and 2 images reporting Android 16 "
         "and 17. The remaining 3 corpora are extractions that carry no /data/system content at "
         "all rather than devices without the store, which was checked by listing their members. "
         "The store is not simply superseded at Android 14: DexManager.notifyDexLoadInternal "
         "still calls PackageDexUsage.record at android-14.0.0_r1, and 2 of the images reporting "
         "Android 14 carry a populated file whose recorded modification time is within days of "
         "the extraction. Why the file stops being maintained on some Android 14 and 15 devices "
         "and not others was not established here, and because the format carries no times it "
         "cannot be settled from the file itself. One set of rows is reported per store found, "
         ...
         "here is laid out. A file matching the pattern that does not begin with the format's "
         "header line is logged and not read. The two Dex Usage (Legacy) artifacts partition the "
         "file without overlap: this one and Cross-Package Code Loads. Every line of the file is "
         "reported across them.",
        "paths": ('*/system/package-dex-usage.list',),
        "output_types": "standard",
