__artifacts_v2__ = {
    ...
    "paypal_device": {
        ...
        "notes": "One row per app data directory. Configuration Cached is Unix "
                 ...
                 "readable: the app keeps its account state in preference files laid out like "
                 "AndroidX EncryptedSharedPreferences, whose entry names and values are both "
                 "encrypted, with a keyset entry stored beside them that the extraction carries "
                 "no key to unwrap, and it holds its tokens in separately encrypted values "
                 "beside them. Encrypted Preference Files and Encrypted Entries count what is "
                 ...
                 "itself is not counted as an entry. Keep Me Logged In Opt Out is the one "
                 "account state value found stored in the clear beside the encrypted ones on the "
                 "tested samples, as stored. The app's home and hub databases are not reported: "
                 "on the tested samples they held layout templates for the app's home screen "
                 "rather than any record of what the account holder "
