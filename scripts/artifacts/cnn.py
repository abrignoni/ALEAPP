__artifacts_v2__ = {
    "cnnAndroidSearches": {
        ...
        "notes": "Read from files/datastore/search.preferences_pb, an Android DataStore preferences "
                 ...
                 "them that way. A term is a value the app held under its recent_search key; it "
                 "does not establish who typed it. The app's other DataStore files "
                 "(com.cnn.storage, tooltip_prefs, the firebase_session files and "
    ...
    "cnnAndroidArticles": {
        ...
        "notes": "Read from files/datastore/article_meter.preferences_pb and "
                 ...
                 "current article are distinguishable. What the article_meter_set list is used "
                 "for is not established from the store beyond its key name, and it is not a "
                 "complete reading history; the store records no per-article timestamp, so a row "
                 "says nothing about when the article was registered. Meter Reset Date is the "
                 "value stored against last_article_meter_reset_date, an eight digit YYYYMMDD "
                 "value with no time or zone recorded. The identifier is the decoded "
