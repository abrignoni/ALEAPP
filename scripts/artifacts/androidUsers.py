__artifacts_v2__ = {
    "android_users": {
        ...
        "notes": "Read from the user records the platform keeps under the system users folder: "
                 ...
                 "Created, Last Logged In and Last Entered Foreground are Unix milliseconds and "
                 "are the created, lastLoggedIn and lastEnteredForeground attributes as stored. "
                 "Last Entered Foreground records when the device switched to this user, not app "
                 ...
                 "In was not established, so it is reported as stored. The system user is created "
                 "with the device and records a creation value of 0, which is reported as blank "
                 "rather than as 1970. User Type and Flags are reported as stored; the type "
                 "string is the platform's own, such as a full secondary user or a managed "
                 "profile. User Name is blank where the record carries no name element. User ID "
                 "and Serial Number were identical on every tested image and are both reported "
                 "because the platform assigns them separately: a new user takes the lowest id "
                 "not in use, so a removed user's id can come back, while the serial is taken "
                 "from a counter the platform only increments and persists as nextSerialNumber in "
                 "userlist.xml. Reference: Android Open Source Project, UserManagerService.java, "
