__artifacts_v2__ = {
    "keepassdx_database_history": {
        ...
        "notes": "One row per entry in the file_database_history table of the app's own Room store "
                 ...
                 "not established here beyond the field name in FileDatabaseHistoryEntity.kt. "
                 "Quick Unlock Credential Stored is derived from the cipher_database table: it "
                 "is Yes when that table holds a row for the same Database URI. cipher_database "
                 "is read only for the presence of a row against the same Database URI; its "
                 "stored values and cipher parameters are not read or reported here. Two other "
                 "tables in the store are not evidential: room_master_table is Room schema "
                 "bookkeeping and android_metadata holds the locale. The app's shared_prefs "
                 "(com.kunzisoft.keepass.libre_preferences.xml) held only an app-lock timeout "
                 "backup on the tested device and is not parsed. The Room store runs in WAL mode "
                 "and held its rows in the -wal sidecar on the tested device, so the sidecar is "
                 "in the paths and is required. The package name is com.kunzisoft.keepass "
                 "followed by a suffix (.libre on the tested device), so the paths accept any "
                 "suffix.",
