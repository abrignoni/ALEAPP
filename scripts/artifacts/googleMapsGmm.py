...
def get_googleMapsGmm(files_found, report_folder, seeker, wrap_text):
    ...
    data_list_storage = []
    data_list_myplaces = []
    file_found_storage = None
    file_found_myplaces = None
    ...
    for file_found in files_found:
        ...
        if file_found.endswith('gmm_storage.db'):
            try:
                ...
                logfunc(f'[!] Error processing gmm_storage.db: {str(e)}')
        ...
        if file_found.endswith('gmm_myplaces.db'):
            try:
                ...
                logfunc(f'Available tables in gmm_myplaces.db: {tables}')
                
                # Try different possible table names
                table_name = None
                if 'sync_item' in tables:
                    table_name = 'sync_item'
                elif 'places' in tables:
                    table_name = 'places'
                elif 'favorites' in tables:
                    table_name = 'favorites'
                else:
                    # Use first available table if known tables not found
                    if tables:
                        table_name = tables[0]
                        logfunc(f'Using table: {table_name} (sync_item not found)')
                    else:
                        logfunc('No tables found in gmm_myplaces.db')
                        db.close()
                        continue
                
                # Query based on available table
                try:
                    cursor.execute(f'''
                    select 
                    rowid,
                    ...
                    logfunc(f'Error querying {table_name}: {str(e)}')
                    db.close()
                    continue
                ...
                for row in all_rows:
                    try:
                        ...
                        # Handle cases where syncitem might be None or invalid
                        if syncitem is None:
                            label = "Unknown"
                            address = ""
                            url = ""
                            ...
                            try:
                                ...
                                if keystring == "0:0":
                                    ...
                                    label = pb[0].get('6', {}).get('7', b'').decode('utf-8', errors='ignore')

                                address = pb[0].get('6', {}).get('2', b'').decode('utf-8', errors='ignore')
                                url = pb[0].get('6', {}).get('6', b'').decode('utf-8', errors='ignore')
                            except Exception as e:
                                logfunc(f'Error decoding protobuf for row {id}: {str(e)}')
                                label = "Unknown"
                                address = ""
                                url = ""
                        ...
                        logfunc(f'Error processing row {id}: {str(e)}')
                        continue
                
                db.close()
            except Exception as e:
                logfunc(f'[!] Error processing gmm_myplaces.db: {str(e)}')
        
    if data_list_storage and file_found_storage:
    ...
    if data_list_myplaces and file_found_myplaces:
