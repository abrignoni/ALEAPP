__artifacts_v2__ = {
    "filemanagerplus_thumbnails": {
        ...
        "description": "Thumbnail images recovered from these file managers' cache",
        ...
        "notes": "One row per cached thumbnail under "
                 ...
                 "The thumbnail is a JPEG the app rendered for the item, so its presence is "
                 "evidence the app processed that item; whether it was displayed to a person is "
                 "not established. **Whether it outlives the file it was made from is not "
                 "guaranteed, and both outcomes were measured on the tested device.** A "
                 "thumbnail whose source no longer exists anywhere on shared storage was found "
                 "still cached and still rendering, which is the case that makes this cache "
                 "worth reading. Against that, deleting a source image and then running the app "
                 "removed exactly that file's thumbnail, taking the cache from five entries to "
                 "four, so the app does prune. The mechanism that decides which outcome occurs "
                 "was not established here and is not claimed. What follows for an examiner is "
                 "the useful part: a thumbnail present is evidence, while an absent thumbnail "
                 "supports no conclusion either way, since it may never have been made or may "
                 "have been pruned since. Kind is taken from the containing directory, which the "
                 "app splits into dir_image, dir_video and dir_audio; on the tested image all "
                 "rows were dir_image, and the video and audio directories existed and were "
                 "empty, which is recorded rather than hidden. The cache file name "
    ...
    "filemanagerplus_scanned_folders": {
        ...
        "notes": "One row per record in Android/data/<package>/files/scanfile.full or "
                 ...
                 "that time against it. It is not proof a person opened that folder. The two "
                 "scan files differ, but barely: counted "
