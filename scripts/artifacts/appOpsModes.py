__artifacts_v2__ = {
    "appops_modes": {
        ...
        "notes": "A row is an op element in system/appops.xml carrying a stored mode, with the package or the "
                 ...
                 "being in effect. Checked against the data and not left on the source alone: not one of the "
                 "14,046 stored modes equals the default the AOSP table gives for that op. Nothing on the op "
                 "element records what set a mode, or when, and the timestamps in the v=\"1\" file belong to its "
                 "access records rather than to the modes. Reference: setUidMode and setPackageMode, "
                 ...
                 "which no tested image exercised and which was confirmed on a constructed file. Across the "
                 "tested images the stored modes were IGNORED on 12,264 rows, FOREGROUND on 1,208, ALLOWED on "
                 "568 and ERRORED on 6; DEFAULT did not appear on any row and no value fell outside the set. "
                 "Reference: MODE_ALLOWED, MODE_IGNORED, MODE_ERRORED, MODE_DEFAULT and MODE_FOREGROUND, "
                 ...
                 "is resolved here from system/packages.xml: 13,785 rows. That lookup uses identity "
                 "packages.xml records: the userId attribute of a package element, on 10,922 rows, or the name "
                 "of the shared-user element when the uid is declared as a shared user id, on 1,514. A shared "
                 "user id is not a package name even when it looks like one: of the shared-user entries on the "
                 "tested images 144 covered more than one package and 380 covered one or none, and of the "
                 "single-member ones the id differed from that package's name on 331 and matched it on 49. A "
                 "UID is split into an Android user and an app id first, "
                 ...
                 "from the stored UID on every other row. It held one value on 11 of the 16 images and more "
                 "than one on 5, and across the tested images the value was 0 on 12,153 rows, 10 on 1,239 and "
                 "150 on 654. A second Android user only appears here once it has a stored mode, so a single "
                 "value is not evidence that the device had a single Android user.\n"
                 ...
                 "not define are left unnamed and the number stays in Op Code, confirmed on a constructed file "
                 "because no tested image exercised it: the 59 distinct codes seen across the tested images "
                 "run from 0 to 133 and the table names every one. The extraction of that table asserted that "
                 "the number of entries equalled the number of Builder occurrences in the array, because a "
                 "pattern that skips an entry shifts every later name; index 96 is a retired placeholder that "
                 "carries an empty name and is left unmapped. The codes are the same in that table at the "
                 "android-14.0.0_r1 and android-15.0.0_r1 tags and at main, so the names do not depend on the "
                 "release. Reference: "
                 ...
                 "That store is misc_de/<user>/apexdata/com.android.permission/access.abx, whose "
                 "app-id-app-ops and package-app-ops sections hold the modes. It is present on 21 of the "
                 "corpora, and permissionAccessState.py reads those two sections, as App Op Modes "
                 "(Permission Store), and its app-id-permissions section as Permission Grants (Permission "
                 "Store).\n"
                 "One of the tested images carries both files, an Android 15 phone, and on it the two agree: "
                 "of the 951 uid and app id entries they share, all 951 record the same mode, and of the 23 "
                 "package entries they share, all 23. Each also holds entries the other does not. appops.xml "
                 "held 14 the newer store did not, every one for a single app id that the same extraction's "
                 "packages.xml no longer names, and the newer store held 6 for six app ids that packages.xml "
                 "does name, so on that device appops.xml was not the more current of the two. Whether that "
                 "holds elsewhere is not established, because no other tested image carries both.\n"
                 ...
                 "a u attribute, and on one of them the uid elements under pkg carry a p attribute holding "
                 "true or false. Neither is written by the AppOpsService or AppOpsCheckingServiceImpl writers "
                 "at android-10.0.0_r1, android-11.0.0_r1, android-13.0.0_r1 or android-15.0.0_r1, neither has "
                 "a documented meaning, and neither is reported.",
