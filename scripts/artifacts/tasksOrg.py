__artifacts_v2__ = {
    "tasks_org_tasks": {
        ...
        "notes": "One row per entry in the tasks table of databases/database, the app's Room "
                 ...
                 "hasDueTime rule (Task.kt at the same commit: dueDate greater than 0 and "
                 "dueDate modulo 60000 greater than zero), which flags a due date as carrying a "
                 "time when its millisecond value is not on a whole-minute boundary; the tested "
                 "due date carried a time and this read Yes, and a date-only due date would read "
                 "No with the time component not meaningful. Recurrence is the task's repeat "
                 "rule as an RRULE string. Parent Task ID is the _id of the parent task for a "
                 "subtask and is blank for a top-level task. Time Estimated and Time Tracked are "
                 "seconds from the app's timer feature. On the tested single-task device Start "
                 "Date, Completed, Deleted, Recurrence and Parent Task ID were empty because "
                 "those features were not used. A second database at databases/tasks.db was "
    ...
    "tasks_org_locations": {
        ...
        "notes": "One row per entry in the places table of databases/database, left joined to the "
                 ...
                 "and a Radius in metres (the geofence radius, 250 on the tested device). On the "
                 "tested device an address searched in the app's location picker was stored as "
                 "1600 Pennsylvania Avenue Northwest at 38.897684, -77.036574 with a 250 m "
                 "radius. Arrival and Departure are the geofence trigger flags from the "
                 "geofences table, reported as Yes when set; both were No on the "
