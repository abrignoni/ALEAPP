__artifacts_v2__ = {
    "poweramp_library": {
        ...
        "notes": "One row per row of folder_files in "
                 ...
                 "Resume Position (ms) is the offset the app would resume from, reported as "
                 "stored; that a complete play resets it to 0 is not among the measurements "
                 ...
                 "the meaning of that value is not sourced, and it is reported as stored. A row "
                 "is evidence the app indexed the file, and a non-zero Play Count is evidence it "
                 "played it to the end; neither is evidence a person was listening. Tables in "
                 "the same database that are not parsed here: eq_presets (17,730 rows), "
                 "milk_presets (263), reverb_presets and milk_preset_containers hold preset "
                 "rows; playlists, playlist_entries, queue, bookmarks, search_history, "
                 "settings_search_history, cat_stats, known_devices and lrc_files were all empty "
                 "on the tested image and hold user activity worth parsing on a device that has "
                 "used those features; albums, artists, genres, composers and years hold lookup "
                 "rows for the "
