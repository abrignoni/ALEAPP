__artifacts_v2__ = {
    "package_dex_usage_app_code": {
        ...
        "notes": "Read from the ART Service dex use store at /data/system/package-dex-usage.pb. Reference: "
         ...
         "or split APK. A row records that the package's code was loaded, which happens when one "
         "of its processes starts; it does not record what caused that to happen, and a process "
         ...
         "store keeps one record per dex file and loading package and overwrites its timestamp, so "
         "a row gives the most recent load and not a history of loads. The platform writes the "
         "file through a debouncer with a 15 second minimum interval and also on the shutdown "
         "broadcast, so a load in the last seconds before acquisition can be absent. Isolated "
         "Process is the record's isolated_process flag, which the platform sets for a load into "
         "an isolated process. It was true on 21 of the 4347 rows of this artifact across the "
         "tested images and false on the rest, so a true value is uncommon rather than absent. An "
         "entry outlives the package it names until the platform's cleanup pass runs: "
         "DexUseManagerLocal.cleanup() drops entries whose owning or loading package is no longer "
         "installed, and nothing inside that class calls it. Measured on an Android 15 emulator "
         ...
         "overlap: this one, Cross-Package Code Loads, and Secondary Dex Loads. Every field of "
         "every message in the schema is reported across them, so nothing in the store is left "
         "unread.",
    ...
    "package_dex_usage_cross_package": {
        ...
        "notes": "Read from the ART Service dex use store at /data/system/package-dex-usage.pb. Reference: "
         ...
         "produced across the tested images. The store keeps one record per dex file and loading "
         "package and overwrites its timestamp, so a row gives the most recent load and not a "
         "history of loads. The platform writes the file through a debouncer with a 15 second "
         "minimum interval and also on the shutdown broadcast, so a load in the last seconds "
         "before acquisition can be absent. One package loading another's code is ordinary "
         "platform behaviour far more often than not: of the 2232 rows across the tested images, "
         "1159 name Google Play services as the owning package and 458 the Android System WebView, "
         "which apps load to render web content, so the row worth attention is an unexpected "
         "pairing rather than the presence of pairings. 1549 of the rows load a file under "
         "/data/app and 683 a platform path under /system, /apex, /product, /vendor or "
         "/system_ext. Isolated Process is the record's isolated_process flag, set for a load into "
         "an isolated process; it was true on 3 of these 2232 rows across the tested images. An "
         "entry outlives the package it names until the platform's cleanup pass runs: "
         "DexUseManagerLocal.cleanup() drops entries whose owning or loading package is no longer "
         "installed, and nothing inside that class calls it. Measured on an Android 15 emulator "
         "whose package list no longer carried a test package that had been removed with pm "
    ...
    "package_dex_usage_secondary": {
        ...
        "notes": "Read from the ART Service dex use store at /data/system/package-dex-usage.pb. Reference: "
         ...
         "history of loads. The platform writes the file through a debouncer with a 15 second "
         "minimum interval and also on the shutdown broadcast, so a load in the last seconds "
         "before acquisition can be absent. Android User is the record's user_id. The platform "
         "stores it in a wrapper that is present but empty when the value is zero, so it is read "
         "as zero rather than left blank for the first user. It was 0 on the 25 tested images "
         "carrying rows here and additionally 10, a second Android user, on 18 of them. Class "
         "Loader Context is the record's class_loader_context, the dependency chain the loader was "
         "given, as stored; it carried a value on all 1568 rows of the tested images. ABI is the "
         "record's abi_name; it was arm64-v8a on all 1568 rows of the tested images, which are 64 "
         "bit ARM devices, and a load for another ABI would be recorded here. Isolated Process was "
         "false on all 1568 of these rows across the tested images. An entry outlives the package "
         "it names until the platform's cleanup pass runs: DexUseManagerLocal.cleanup() drops "
         "entries whose owning or loading package is no longer installed, and nothing inside that "
         "class calls it. Measured on an Android 15 emulator whose package list no longer carried "
         "a test package that had been removed with pm uninstall: the store still named that "
