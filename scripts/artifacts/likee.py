            ...
            with open(outputpath, 'w') as w:
                ...
                for line in Lines:
                    if "address" in line:
                        ...
                        dstring1 = (line_decode[:line.find("address",0,6)]).replace(u'\uFFFD'u'\u2660', " ")
...
def get_likee_db(files_found, report_folder, seeker, wrap_text, time_offset):    
    ...
    if usageentries > 0:
        ...
        report.write_artifact_data_table(db1_data_headers, db1_data_list, src_likee_pub, html_escape=False)
        report.end_artifact_report()
    ...
    for likee_msg_db in db2_data_list:
        ...
        if usageentries > 0:
            ...
            report.write_artifact_data_table(db2_data_headers, db2_data_list, src_likee_msg, html_escape=False)
            report.end_artifact_report()
...
"likee_location": (
    "LIKEE",
    ('*/video.like/files/*/*location.kv'),
    get_likee
),
"likee_databases": (
    "LIKEE",
    ('*/video.like/databases/*.db'),
    get_likee_db
)
}
