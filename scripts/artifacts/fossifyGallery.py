__artifacts_v2__ = {
    "fossify_gallery_media": {
        ...
        "notes": "One row per entry in the media table of databases/gallery.db. This table is the app's own "
                 ...
                 "files it was image on every row. Video Duration is 0 for a still image and was "
                 "0 on every tested row. Deleted is set from deleted_ts; on the tested device it "
                 "was non-zero on the file moved to the app's recycle bin, and that file's Full "
                 "Path was stored with a recycle_bin/ prefix, so a non-empty Deleted value marks "
                 "a file the app placed in its recycle bin. Whether the file's bytes remain on "
                 "the device is not established from this table. Date Taken, Last Modified and "
                 "Deleted are all Unix milliseconds and are reported as UTC; on the tested "
                 "device the deletion time 18:00 UTC matched the device's 2:00 PM local action. "
                 "The Favorite flag was confirmed against a favourited file, and the deleted "
                 "state against a file moved to the recycle bin. Three related tables, "
                 "favorites, date_takens and directories, are not parsed here. The app is the "
                 "maintained successor to Simple "
